...
Vector-boson scattering (\ac{VBS}) processes provide a unique experimental
signature, producing
...
tag jets allows measurements to be conducted at lower cross sections than
are accessible
%would otherwise be accessible
%in the current dataset
for similar processes with less unique topology.
...
only \ac{SM} contributions at \ac{LO}.
...
\begin{figure}
  ...
  \caption{
    ...
    black circle contains the multiboson interaction, which for a leading-order
    \acs{SM} interaction will be one of the two shown (right).
  }
  \label{fig:vbs-vbsfeynman}
\end{figure}

%Possible NP enhancements
Beyond the chance to study the rare \ac{SM} vertices that contribute to this
interaction, this analysis also offers sensitivity to any potential new physics
that modifies or adds \acp{TGC} or \acp{QGC} \cite{DasBakshi2022}. These
anomalous couplings would cause a deviation in the rate from the \ac{SM}
expectation which, if sufficiently large, would be measured in this analysis.
...
from \QCDZy production. Figure \ref{fig:vbs-qcdfeynman} gives example
