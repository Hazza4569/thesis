...
Given the baseline selection from Section \ref{sec:vzy-selection} and the
\ac{BDT} discriminant developed in Section \ref{sec:vzy-bdt}, additional
selection cuts can be applied to define signal and control regions for use in
the fit. By further constraining the analysis region in constructing the
\ac{SR}, the sensitivity to the analysis to the signal process is improved.
...
%TODO explain blinded analysis strategy in anacom?
...
\begin{figure}[tbh]
  ...
  \caption{
    ...
    are shown stacked atop one another. The error band represents the total
...
The $m_{jj}$ cut defining the on-peak region was roughly optimised by
...
scan.
A gap is included between the on-peak region and the sidebands to
minimise signal leakage; this is chosen such that no more than 5\% of the total
signal events fall in the sideband regions.
Figure \ref{fig:vzy-srcr-mjjcuts} shows the $m_{jj}$ cuts employed, in the
context of the shape of the signal and background distributions.

\begin{figure}[tbhp]
  \centering
  \includegraphics[width=.95\textwidth]{\resource{m_jj_window_scan.pdf}}
  \caption{
...
\begin{figure}[tbhp]
  \centering
  \includegraphics[width=.95\textwidth]{\resource{m_jj_cut_schematic.pdf}}
  \caption{
