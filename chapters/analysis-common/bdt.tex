...
%discriminant that can be applied to any other event. The \ac{BDT} can
...
rarity distribution is defined as
%
\begin{equation*}
  \mathcal{R}^\text{sig}(y) = \int_{-\inf}^{y} \hat{y}_B(y') dy',
\end{equation*}
%
where $y$ is the \ac{BDT} score and $\hat{y}_B$ is the distribution of the \ac{BDT}
score for background events in the training sample \cite{TMVAguide}.
This variable takes values from 0 to 1, and is constructed
such that background events should be uniformly distributed in signal rarity,
whereas signal events occur more frequently at values closer to 1.
This distribution is used for the \ac{BDT}
