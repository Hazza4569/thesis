...
of the \ac{LHC} in Run 4, the trigger needs to be further improved to operate in
...
will continue to contribute to the trigger, and will refine decisions made by
employing additional information: information from a larger area than typically
available to a single \ac{FEX} but also finer in granularity.
...
\begin{figure}
  ...
  \caption{
    ...
    being the main change with respect to the Phase-I system.
...
\ac{eFEX}, with energies in each individual cell without them being grouped into
SuperCells. This gives a 4-8 times increase in granularity, depending on the
region of the calorimeter.
% Discuss how GEP and eFEX interact

The additional information available to the \ac{GEP} means it can work together
with the eFEX. The \ac{eFEX} will create \acp{TOB} with associated variables
(discussed in Section \ref{sec:trig-upgrade-p1}) which are sent to the \ac{GEP},
which can then further probe the same region of the calorimeter to determine if
the candidate object should be accepted. The algorithms used by the \ac{GEP} to
do this are the topic of the study in Section \ref{sec:trig-eratio}.
